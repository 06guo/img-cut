...
// window.fshowsScrollTop
// 这个变量是为了解决h5页面是整屏的时候,没有滚动条,只有在div内部滚动的bug,
// 在没有滚动条的时候,每次跳转页面,div都会充值到顶部,原因是因为vueg只做了兼容有滚动条的时候的情况,
/**
 * @typedef Options
 * @type {Object}
  ...
  install (Vue, router, options = {}) {
    ...
    Vue.mixin({
      ...
      beforeRouteLeave (ro, from, next) {
        if (document.getElementById('order-scroll') && document.getElementsByClassName('fshows-scroll')[0]) {
          window.baseTop = document.getElementById('order-scroll').getBoundingClientRect().top || 0
          window.fshowsScrollTop = document.getElementsByClassName('fshows-scroll')[0].getBoundingClientRect().top || 0
        } else {
          window.baseTop = 0
          window.fshowsScrollTop = 0
        }
        lastComponent = this
        next()
      },
    ...
    router.beforeEach((to, from, next) => {
      ...
      if (toDepth === fromDepth) {
        ...
        // 深度相同时禁用动画
        if (op.disableAtSameDepths) { transitionType = null }
        lastPath = from.path
      }
      ...
      // 处理map选项
      const enter = Object.keys(op.map).find(key => op.map[key].enter && op.map[key].enter.indexOf(from.name) > -1)
      const leave = Object.keys(op.map).find(key => op.map[key].leave && op.map[key].leave.indexOf(to.name) > -1)
      if (enter && enter === to.name) {
        transitionType = 'back'
      } else if (leave && leave === from.name) {
        transitionType = 'back'
      } else if (Object.keys(op.map).indexOf(from.name) > -1) {
        if (op.map[from.name]['enter'] && op.map[from.name]['enter'].indexOf(to.name) > -1) {
          transitionType = 'forward'
        }
        if (op.map[from.name]['leave'] && op.map[from.name]['leave'].indexOf(to.name) > -1) {
          transitionType = 'back'
        }
        if (op.map[from.name]['disable'] && op.map[from.name]['disable'].indexOf(to.name) > -1) {
          transitionType = null
        }
      } else if (Object.keys(op.map).indexOf(to.name) > -1) {
        if (op.map[to.name]['leave'] && op.map[to.name]['leave'].indexOf(from.name) > -1) {
          transitionType = 'forward'
        }
        if (op.map[to.name]['enter'] && op.map[to.name]['enter'].indexOf(from.name) > -1) {
          transitionType = 'back'
        }
        if (op.map[to.name]['disable'] && op.map[to.name]['disable'].indexOf(from.name) > -1) {
          transitionType = null
        }
      }
    ...
    function addEffect (instance = this, el) {
      ...
      // 移除背景
      if (bg && bg.getElementsByClassName('fshows-scroll')[0]) {
        bg.getElementsByClassName('fshows-scroll')[0].style.transform = `translateY(${window.fshowsScrollTop - window.baseTop}px)`
      }
      // 动画完成后移除class
      setTimeout(() => {
        ...
        // 移除背景
        if (bg && bg.getElementsByClassName('fshows-scroll')[0]) {
          bg.getElementsByClassName('fshows-scroll')[0].style.transform = `translateY(0px)`
          bg.innerHTML = ''
        }
